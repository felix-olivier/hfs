function [wavenumbers, intensity, resolved]=create_hfpat_gen(wavenrJ, Ju, Jl, Au, Bu, Al, Bl, Iqnr)
...
for i=1:length(Fu)
    for j=1:length(Fl)
        ...
        wavenr(transition_index)=wavenrJ+(0.5*Au*Ccalc_gen(Fu(i),Ju, Iqnr))-(0.5*Al*Ccalc_gen(Fl(j),Jl, Iqnr))+(Bu*function_calc_gen(Fu(i),Ju, Iqnr))-(Bl*function_calc_gen(Fl(j),Jl, Iqnr)); %contains wavenr of each transition
